...
resource "aws_security_group" "frontend_server" {
  ...
  tags = {
    Name = "${var.tags}-frontend-server-sg"
  }
}
...
resource "aws_security_group" "backend_server" {
  ...
  tags = {
    Name = "${var.tags}-backend-server-sg"
  }
}
