...
resource "aws_autoscaling_group" "frontend" {
  name_prefix          = "${var.tags}-frontend-asg"
  ...
  tag {
    key                 = "Name"
    value               = "${var.tags}-frontend-asg"
    propagate_at_launch = true
  }
}

resource "aws_autoscaling_policy" "frontend_scale_out" {
  name                   = "${var.tags}-frontend-scale-out"
...
resource "aws_autoscaling_group" "backend" {
  name_prefix          = "${var.tags}-backend-asg"
  ...
  tag {
    key                 = "Name"
    value               = "${var.tags}-backend-asg"
    propagate_at_launch = true
  }
}

resource "aws_autoscaling_policy" "backend_scale_out" {
  name                   = "${var.tags}-backend-scale-out"
